...
The navigation is structured around a bottom tab bar consisting of Home, Leads, Escalations and Follow-ups with the Conversation Detail pushed as a stack screen on top of the tabs.

* **Home Dashboard** — Displays our central control stats for the Leads, Escalated, Follow-ups and Missed. Quick action buttons to simulate inbound inquiries and a live activity feed showing recent system events.
...
Instead of cluttering components with verbose React Native `StyleSheet` objects, I went with NativeWind (Tailwind CSS). Inline utility classes are faster to write, make design consistency across components automatic, and keep screen files highly readable during code reviews. It also makes adjusting responsive layouts and paddings for small vs large devices trivial.
...
* All data is simulated locally via the mock layer as there is no active live backend sync.
