...
def process_enquiry(enquiry_id: str, db: Session):
    ...
    1. Fetches the enquiry
    2. Tries to match an SOP
    3. Updates the record with result
    4. Adds an AI message and a timeline event
    5. Auto-escalates if no SOP matched
    """
    ...
    if matched_sop:
        ...
        logger.info(
            "SOP matched", extra={"enquiry_id": enquiry_id, "sop": matched_sop["name"]}
        )
        ...
        enquiry.updated_at = datetime.utcnow()
        _add_timeline(db, enquiry_id, "escalated", "Auto-escalated: no SOP match found")
